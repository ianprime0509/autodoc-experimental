(function() {
    ...
    function renderDecl(decl_index, original_decl_index = decl_index) {
      ...
      switch (category) {
        ...
        case CAT_function:
          return renderFunction(decl_index, original_decl_index);
        case CAT_type_function:
          return renderTypeFunction(decl_index);
        case CAT_error_set:
          return renderErrorSetPage(decl_index);
        case CAT_alias:
          return renderDecl(wasm_exports.get_aliasee(), original_decl_index);
        default:
    ...
    function renderFunction(decl_index, original_decl_index) {
      renderNav(original_decl_index);
      renderDeclHeading(decl_index);
      renderTopLevelDocs(decl_index);
      renderParams(decl_index);
      renderDocTests(decl_index);

      domFnProtoCode.innerHTML = fnProtoHtml(decl_index, original_decl_index, false);
    ...
    function renderNamespace(base_decl, members, fields) {
      ...
      member_loop: for (const member of members) {
        let resolved = member;
        while (true) {
          ...
          switch (resolvedCategory) {
            ...
            case CAT_primitive:
              ...
              continue member_loop;
            case CAT_alias:
              resolved = wasm_exports.get_aliasee();
              continue;
            default:
              throw new Error("unknown category: " + resolvedCategory);
          }
        }
      }
      ...
      if (fnsList.length !== 0) {
          ...
          for (let i = 0; i < fnsList.length; i += 1) {
              ...
              const resolved = fnsList[i].resolved;
              ...
              protoCodeDom.innerHTML = fnProtoHtml(resolved, decl, true);
    ...
    function byDeclIndexName2(a, b) {
      const a_name = declIndexName(a.member);
      const b_name = declIndexName(b.member);
      return operatorCompare(a_name, b_name);
    }
    ...
    function fnProtoHtml(decl_index, base_decl_index, linkify_fn_name) {
      return unwrapString(wasm_exports.decl_fn_proto_html(decl_index, base_decl_index, linkify_fn_name));
    }
